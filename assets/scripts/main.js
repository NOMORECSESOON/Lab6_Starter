...
function addRecipesToDocument(recipes) {
  ...
  var main_ele = document.querySelector('main');
  // A11. TODO - Loop through each of the recipes in the passed in array,
  //            create a <recipe-card> element for each one, and populate
  //            each <recipe-card> with that recipe data using element.data = ...
  //            Append each element to <main>
  for (const sin_ele in recipes) {
    let curr = document.createElement('recipe-card');
    curr.data = recipes[sin_ele];
    main_ele.append(curr);
  }
}
...
function initFormHandler() {
  ...
  function func_form_ele(){
    var Data0 = new FormData(to_form_element);
    var recipeObject = {};
    for (var pair of Data0.entries()) {
      recipeObject[pair[0]] = pair[1];
    }

  // B6. TODO - Create a new <recipe-card> element
  var new_element = document.createElement("recipe-card");
  // B7. TODO - Add the recipeObject data to <recipe-card> using element.data
  new_element.data = recipeObject;
  // B8. TODO - Append this new <recipe-card> to <main>
  var main_ele = document.querySelector('main');
  main_ele.append(new_element);
  ...
  getDeleteButton.addEventListener("click", clear_all_item);
